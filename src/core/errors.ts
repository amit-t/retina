...
//
// All errors surfaced to the HTTP layer extend `RetinaError` so the error
// middleware can map them to a stable JSON envelope with consistent
// `code` + `status`. The mapping here must stay in sync with the spec's
// §Error handling table:
//
//   docs/superpowers/specs/2026-04-21-retina-image-api-design.md
//
// Do not introduce new error classes outside this file — keeping the list
// here means the middleware, tests, and spec stay in lockstep.
...
export class ImageTooLargeError extends RetinaError {
  constructor(message = 'image exceeds maximum size', options: RetinaErrorOptions = {}) {
...
export class ImageFetchError extends RetinaError {
  constructor(message = 'failed to fetch image', options: RetinaErrorOptions = {}) {
